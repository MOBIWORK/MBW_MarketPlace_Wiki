...
class WikiPage(WebsiteGenerator):
    ...
    def on_trash(self):
        frappe.db.sql(
            "DELETE FROM `tabWiki Page Revision Item` WHERE wiki_page = %s", self.name)
        ...
        wiki_sidebar_name = frappe.get_value(
            "Wiki Group Item", {"wiki_page": self.name})
    ...
    def verify_permission(self, permtype):
        ...
        if not permitted:
            ...
            frappe.local.response["location"] = "/login?" + \
                urlencode({"redirect-to": frappe.request.url})
        ...
        if frappe.form_dict:
            ...
            if splits:
                for index, _route in enumerate(splits[:-1], start=1):
                    ...
                    if wiki_page:
                        parents.append(
                            {"route": "/" + full_route, "label": wiki_page[0].title})
    ...
    def get_context(self, context):
        ...
        wiki_space_name = frappe.get_value(
            "Wiki Group Item", {"wiki_page": self.name}, "parent")
        ...
        context.page_toc_html = (
            self.calculate_toc_html(
                html) if wiki_settings.enable_table_of_contents else None
        )
        ...
        revisions = frappe.db.get_all(
            ...
            fields=["content", "creation", "owner",
                    "name", "raised_by", "raised_by_username"],
        )
        ...
        if len(revisions) > 1:
            ...
            context.previous_revision = {
                "content": "<h3>No Revisions</h3>", "name": ""}
        ...
        if (frappe.form_dict.editWiki or frappe.form_dict.newWiki) and frappe.form_dict.wikiPagePatch:
            context.patch_new_code, context.patch_new_title = frappe.db.get_value(
                "Wiki Page Patch", frappe.form_dict.wikiPagePatch, [
                    "new_code", "new_title"]
            )
    ...
    def get_sidebar_items(self):
        wiki_sidebar = frappe.get_doc(
            "Wiki Space", {"route": self.get_space_route()}).wiki_sidebars
        sidebar = {}
        ...
        for sidebar_item in wiki_sidebar:
            ...
            if not wiki_page.allow_guest:
                permitted = frappe.has_permission(
                    wiki_page.doctype, "read", wiki_page)
                if not permitted:
                    continue
    ...
    def clone(self, original_space, new_space):
        ...
        cloned_wiki_page.route = cloned_wiki_page.route.replace(
            original_space, new_space)
...
def update_page_settings(name, settings):
    ...
    frappe.has_permission(doctype="Wiki Page",
                          ptype="write", doc=name, throw=True)
    clear_sidebar_cache()
    settings = frappe.parse_json(settings)

    frappe.db.set_value(
        "Wiki Group Item", {"wiki_page": name}, "hide_on_sidebar", sbool(
            settings.hide_on_sidebar)
    )
